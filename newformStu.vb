...
Public Class newformStu
    Private Sub newformStu_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        'TODO: 这行代码将数据加载到表“SMSDataSet5.Student”中。您可以根据需要移动或移除它。
        Me.StudentTableAdapter.Fill(Me.SMSDataSet5.Student)
        Me.Width = 800
        Me.Height = 500
    ...
    Private Sub InsertButton_Click(sender As Object, e As EventArgs) Handles InsertButton.Click
        ...
        For Each row As DataGridViewRow In DataGridView1.Rows
            If Not row.IsNewRow AndAlso Not String.IsNullOrWhiteSpace(Convert.ToString(row.Cells(0).Value)) Then ' 检查每行的第一个单元格是否为空
                ...
                If Not hasEmptyCell Then
                    Dim Sno As String = Convert.ToString(DataGridView1.CurrentRow.Cells(0).Value) ' 获取 DataGridView 中当前选中行的单元格值
                    Dim Cno As String = Convert.ToString(DataGridView1.CurrentRow.Cells(1).Value)
                    Dim Sname As String = Convert.ToString(DataGridView1.CurrentRow.Cells(2).Value)
                    Dim EnrollYear As String = Convert.ToString(DataGridView1.CurrentRow.Cells(3).Value)
                    Dim BirthYear As String = Convert.ToString(DataGridView1.CurrentRow.Cells(4).Value)
                    Dim Major As String = Convert.ToString(DataGridView1.CurrentRow.Cells(5).Value)
                    If Insertinto(Sno, Cno, Sname, EnrollYear, BirthYear, Major) Then ' 调用插入学生记录的方法，并传入学生信息
                        successCount += 1
                    End If
                End If
            End If
        ...
        If successCount > 0 Then
            MessageBox.Show($"成功插入 {successCount} 条学生记录！", "成功", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
